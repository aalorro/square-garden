...
class ProgressRepository(private val context: Context) {
    ...
    val diagonalTokensFlow: Flow<Int> = context.dataStore.data.map { prefs ->
        prefs[DIAGONAL_TOKENS_KEY] ?: 0
    }

    suspend fun addDiagonalToken() {
        context.dataStore.edit { prefs ->
            prefs[DIAGONAL_TOKENS_KEY] = (prefs[DIAGONAL_TOKENS_KEY] ?: 0) + 1
        }
    }

    suspend fun useDiagonalToken(): Boolean {
        var success = false
        context.dataStore.edit { prefs ->
